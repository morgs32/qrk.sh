...
import { authenticatedVersionedAggregateRepoFixedDORepoConfig } from './authenticatedVersionedAggregateRepoFixedDORepoConfig.js';
...
export class AuthenticatedVersionedAggregateRepo extends makeFixedDORepo({
  namespaceBinding: 'AUTHENTICATED_VERSIONED_AGGREGATE_REPO',
  fixedDORepoConfig: authenticatedVersionedAggregateRepoFixedDORepoConfig,
}) {
  static override readonly fixedDORepoConfig =
    authenticatedVersionedAggregateRepoFixedDORepoConfig;
  ...
  readonly #deltas = makeOutboxQueue({
    name: 'deltas',
    db: this.db,
    outboxTable: authenticatedVersionedAggregateRepoDbConfig.schema.deltas,
  ...
  /*
   * Exposes the already bound deltas capability from AuthenticatedVersionedAggregateRepo.
   *
   ...
   * that cursor, then reconcile requested command outcomes from AVAC.
