# Map catalog and custom payload controls design
...
8. `makeCatalog` preserves `payloadForm` alongside the payload shape, data shape, default data, callback, and sizes.
...
22. Register catalog `map`, variant `place`, and size `4x4`.
...
1. Extend the existing `makeVariant` unit and compile-time seam to prove key validity, decoded value inference, required defaults, catalog preservation, controlled defaults, and unchanged static/text fallback behavior.
