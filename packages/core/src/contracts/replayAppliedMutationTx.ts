...
export const prepareReplayAppliedMutation = Effect.fn(
  ...
  if (sameNameCurrentModel !== undefined) {
    ...
    switch (mutation.operationName) {
      ...
      case 'replicateResource':
        ...
        currentMutationSchema = sameNameCurrentModel.replicateResourceMutation(
          sameNameCurrentModel.version,
        );
        break;
      case 'update':
  ...
  if (targetMutation === undefined) {
    ...
    const invokeAdapter = yield* Schema.decodeUnknown(
      Schema.declare(
        (
          input: unknown,
        ): input is (
...
export const replayAppliedMutationTx = Effect.fn('replayAppliedMutationTx')(
  function* <CONFIG extends IDbConfig>(props: {
    ...
    const targetMutation = yield* prepareReplayAppliedMutation(props);
    ...
    const appliedMutation = yield* applyAggregateMutationTx({
      ...
      commandId: props.mutation.commandId,
      mutationIndex: props.mutation.mutationIndex,
      appliedAt: props.mutation.appliedAt,
    });
