...
The project already distinguishes a catalog brick variant by the pair `catalogName` and `brickName`, while a GridItem is a placed instance with layout and persistence identity. The extraction must preserve that distinction and current behavior while giving every catalog brick a stable development URL.

## Solution

Incubate a publishable `@qrk.sh/bricks` package with an internal TanStack Start development sandbox in this monorepo. The package owns all eighteen existing brick catalogs, their catalog metadata, rendering dependencies, shadcn components, compiled stylesheet, and colocated sandbox. The private Next.js application continues to own GridItem placement, persistence, drag-and-drop, drawer behavior, and site routing.

The sandbox consumes only the package's public API. It provides a catalog, catalog pages, and a canonical page for each `(catalogName, brickName)` pair while remaining excluded from the published package tarball. Once the boundary is stable, the package and its development sandbox can move together to an open-source repository without redesigning consumer imports.
...
2. As a brick author, I want to browse all catalogs and the variants within one catalog, so that I can inspect the complete catalog.
...
5. As a private QRK application, I want to import the catalog from `@qrk.sh/bricks`, so that persisted `catalogName` and `brickName` values resolve without private catalog duplication.
...
7. As an existing QRK user, I want all current brick catalogs, including GitHub, Figma, and Image, to keep their current visible behavior after extraction.
...
3. Use `brick`, `catalog`, `brick variant`, and `GridItem` consistently. A brick variant's catalog identity remains the two-part `(catalogName, brickName)` pair; do not introduce a combined block slug.
4. Move all eighteen existing catalogs, `BrickFrame`, catalog types, `makeBrick`, `makeCatalog`, `catalogsHash`, and `findCatalogBrick` into the package.
...
10. Expose one root catalog API containing `catalogsHash`, `findCatalogBrick`, `IBrick`, `ICatalog`, `ICatalogBrick`, and `ICatalogBrickDef`. Export the stylesheet as `@qrk.sh/bricks/styles.css`. Do not add catalog, individual-brick, or internal shadcn subpath APIs.
...
12. Give the sandbox three route surfaces: `/` for all catalogs, `/catalogs/$catalogName` for every variant in one catalog, and `/bricks/$catalogName/$brickName` for one canonical brick preview.
13. Render an individual brick at its exact `def.w` by `def.h` proportions. Provide an adjustable grid-unit size, a light/dark canvas toggle, and visible catalog name, brick name, label, width, and height metadata.
14. Return an explicit not-found route state when either the catalog or brick name is unknown. Do not substitute a fallback brick.
...
2. Verify the catalog page links every catalog, a catalog page renders all of its variants, and every generated brick link uses the two-part route identity.
...
5. Verify unknown catalog and brick names produce the explicit not-found state without rendering a fallback.
...
7. Keep the existing Next.js BrickCatalog, BrickCarousel, BrickDetail, and Grid Playwright suites as the private-application regression seam; they must continue resolving and placing bricks by `catalogName` and `brickName`.
...
3. Combining `catalogName` and `brickName` into one slug or identifier.
