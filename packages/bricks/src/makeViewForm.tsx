import { makeEffectSchema, PrimitiveKind, type InferDecodedRow, type IShape } from "@zerospin/schema";
...
export function makeViewForm<const SHAPE extends IShape>(props: {
  ...
  for (const [name, descriptor] of Object.entries(props.shape)) {
    if (
      ...
      (descriptor.kind !== PrimitiveKind.Boolean || descriptor.nullable !== false ||
        !("defaultValue" in descriptor) || typeof descriptor.defaultValue !== "boolean")
    ) {
      throw new Error(`makeViewForm: ${name} requires a custom form; automatic controls require non-nullable booleans with boolean defaults`);
    }
  ...
  return {
    ...
    decode(value: unknown) {
      const withDefaults = value !== null && typeof value === "object" && !Array.isArray(value)
        ? { ...defaultValue, ...value }
        : value;
      return Schema.decodeUnknownSync(schema)(withDefaults, { onExcessProperty: "error" });
    },
    form({ value, onChange }: { value: unknown; onChange: (value: unknown) => void }) {
      const withDefaults = value !== null && typeof value === "object" && !Array.isArray(value)
        ? { ...defaultValue, ...value }
        : value;
      const decodedValue = Schema.decodeUnknownSync(schema)(withDefaults, { onExcessProperty: "error" });
      if (Form === undefined) {
        return (
