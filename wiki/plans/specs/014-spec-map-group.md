# Map group and custom payload controls design
...
5. As a group user, I want the Map brick to render a useful Chicago default before configuration, so that every group boundary has deterministic content.
...
8. `makeGroup` preserves `payloadForm` alongside the payload shape, data shape, default data, callback, and sizes.
...
22. Register group `map`, variant `place`, and size `4x4`.
...
1. Extend the existing `makeVariant` unit and compile-time seam to prove key validity, decoded value inference, required defaults, group preservation, controlled defaults, and unchanged static/text fallback behavior.
...
3. Extend the existing credentialed Bricks Playwright seam to prove the Chicago default, live autocomplete keyboard selection, controlled place ID, successful `Get data`, updated JSON, updated map marker, clear/no-results/error states, and rendering through group, preview, Grid, and detail boundaries.
