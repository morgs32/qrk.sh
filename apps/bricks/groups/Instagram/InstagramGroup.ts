...
export const instagramGroup = makeGroup({
  ...
  groupDescription: "A public Instagram profile and its latest posts.",
  catalogs: {
    default: makeCatalog({
      catalog: "default",
      catalogName: "Default",
      catalogDescription: "An Instagram profile card with four recent posts.",
      configuration: makeFetcherConfiguration({
        ...
        fetcher: async ({ api, catalogOptions, setData }) => {
          const result = await api.instagramRepo().scrape(catalogOptions.url);
      ...
      order: 0,
      xs: { component: InstagramDefault4x4, w: 4, h: 4 },
    }),
  },
});
