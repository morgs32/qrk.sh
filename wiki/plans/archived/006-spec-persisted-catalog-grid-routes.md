# Persisted catalog grid routes design
...
The brick sandbox currently owns each catalog grid in route-local React state. Added grid bricks and moved positions disappear on reload, and a selected grid brick cannot have a stable route because its `gridBrickId` cannot be resolved after the catalog page remounts.
...
Add one module-level Zustand React store named `useGridStore`. It owns one grid entry per `catalogName`, persists those entries to localStorage through Zustand's `persist` middleware, and stores only JSON-serializable data. Each catalog entry contains its React Grid Layout geometry and the catalog identity associated with each added `gridBrickId`.

Turn the catalog workspace into a routed surface. The catalog route shows the catalog in the left pane, while `/catalogs/$catalogName/gridBrick/$gridBrickId` shows that placed brick's detail view in the left pane. Both routes keep the same persisted grid visible and interactive in the right pane.

## User Stories

1. As a brick author, I want each catalog to retain its own grid, so that switching catalogs does not mix their arrangements.
...
2. Store catalog grids under `catalogName`; every catalog entry owns its own layout and grid-brick identity records.
...
5. Lazily initialize a catalog with the four existing gray two-by-two fixtures. Fixtures remain part of the layout but have no catalog identity and are not selectable.
...
8. Replace catalog-route component state for layout and grid-brick identity with `useGridStore`; active native drag state may remain ephemeral because it exists only during one drag gesture.
9. Use `/catalogs/$catalogName/gridBrick/$gridBrickId` as the placed-brick detail route. `gridBrickId` identifies one placed grid instance, not a catalog variant.
10. Keep the right grid mounted, visible, and interactive on both the catalog index and grid-brick detail routes. Only the left pane route content changes.
...
13. If the catalog exists but the hydrated store does not contain `gridBrickId` for that catalog, render an inline `Grid brick not found` state in the left pane and leave the right grid visible.
14. The detail pane shows the selected brick's catalog identity and full-size preview, with a back action to `/catalogs/$catalogName`.
...
3. Drag a representative catalog brick into the catalog grid, verify its catalog dimensions, move it, select it, and assert navigation to `/catalogs/$catalogName/gridBrick/$gridBrickId`.
