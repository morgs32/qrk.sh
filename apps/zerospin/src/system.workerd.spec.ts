...
describe("QRK system", () => {
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const { userV4 } = yield* Effect.promise(() => import("./aggregates/user/UserV4"));
        const authenticate = userV4.authentication.authenticate;
  ...
  it.effect("registers the user aggregate and enforces its authenticated ownership", () =>
    Effect.gen(function* () {
      ...
      expect(system.aggregates.user["4.0.0"].authentication.pattern.source).toBe("/:clerkUserId");
  ...
  it.effect("rejects malformed signatures and invalid Clerk tokens", () =>
    Effect.gen(function* () {
      ...
      const authentication = system.aggregates.user["4.0.0"].authentication;
