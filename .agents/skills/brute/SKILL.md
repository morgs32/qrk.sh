...
description: >-
  ...
  compile fixes, or passing tests. Use when the user says brute, /brute,
  brute force, just make the change, or wants a fast structural pass and
...
```md
## actor-id migration (brute pass)

1. **`makeActorController` call sites** — still pass removed `actor` prop in `packages/core`, `packages/system-worker`, `apps/apis`
2. **`SystemRepo.authorizeActor`** — still reads `ACTOR['actor']` after `IActorController` shape change
```

Do **not** use `UGLY.md` for brute fallout unless the user also asks for `/ugly`. Structural debt belongs in `TODOS.md` when you record it.
...
3. Do **not** offer to fix types, tests, or downstream call sites unless the user asks to leave brute mode.

## Anti-patterns for brute passes
...
- Treating brute as a **spike** — spikes explore with stubs; brute **applies** the real edit; record debt in `TODOS.md` only when fallout is non-obvious.
...
If scope is unclear, brute the **smallest** named file or symbol; mention blast radius in the reply or `TODOS.md` only when it is not obvious from the edit itself.
