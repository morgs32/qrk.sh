...
import { type authenticatedVersionedAggregateRepoDbConfig } from '../../AuthenticatedVersionedAggregateRepo/authenticatedVersionedAggregateRepoDbConfig.js';
import { authenticatedVersionedAggregateChainDbConfig } from '../authenticatedVersionedAggregateChainDbConfig.js';

/*
 * The replica delta outbox publishes per-command frontend output to AVAC.
...
export const receiveDeltas = Effect.fn(
  'AuthenticatedVersionedAggregateChain.receiveDeltas',
)(function* (props: {
  rows: readonly (typeof authenticatedVersionedAggregateRepoDbConfig.schema.deltas.$inferSelect)[];
  db: IDb;
  key: {
    aggregateId: string;
    ...
    yield* Effect.try({
      try: () =>
        props.db.transaction(tx => {
          ...
          tx.insert(authenticatedVersionedAggregateChainDbConfig.schema.deltas)
            .values({
