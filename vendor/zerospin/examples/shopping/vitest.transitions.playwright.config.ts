...
export default mergeConfig(
  makePlaywrightVitestConfig({
    include: [
      'tests/browser/reactTransitionsAndCommissioning.playwright.spec.ts',
    ],
  ...
  }),
  {
    ...
    server: {
      proxy: {
        '/__transition-control': {
          target: 'http://127.0.0.1:3026',
          rewrite: (requestPath: string) =>
            requestPath.replace(/^\/__transition-control/, ''),
        },
        '/__zerospin': {
    ...
    test: {
      globalSetup: [
        './tests/browser/transition-fixture/transitionFixtureGlobalSetup.ts',
      ],
      hookTimeout: 180_000,
      testTimeout: 600_000,
    },
