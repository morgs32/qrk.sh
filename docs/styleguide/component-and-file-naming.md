...
Shared brick Typeset, `.qrk-bricks` tokens, and brick interaction CSS live in [`bricks.css`](../../apps/library/bricks.css) next to brick components. Each app owns one Tailwind entry: library [`app/globals.css`](../../apps/library/app/globals.css) and studio [`app/globals.css`](../../apps/studio/app/globals.css). Both import `@qrk.sh/library/bricks.css` (or the local alias) and own their own `@theme`. No other library CSS entrypoints. Import each file directly (no barrel).

- **`BrickFrame`** — owned by `makeModule`’s `Brick` renderer (not by presentations). Fill sizing, `qrk-bricks`, `typeset typeset-brick`, and nested SVG `user-select: none` (`[&_svg]:select-none`). No background or text-color props. Typeset owns body size, leading, heading scale, color, and flow. The library app root (`RootLayout`) and portaled drawer content also carry `typeset typeset-brick` so chrome inherits the same scale; do not add `font-mono` / `text-sm` / `text-zinc-*` or other type size/color overrides on those shells or in brick presentations. `.typeset-brick` only wires `--typeset-font-*` to the library fonts.
...
- **`brickTokens.ts`** — inset/gap/icon (`brickInsetClass`, `brickStackGapClass`, `brickMetaIconClass`). Prefer these over ad hoc `p-2` in brick presentations. Do **not** put font-size or font-color `text-*` classnames on module markup; use semantic tags (`h2`/`h3`/`p`) under Typeset and inherit color. There is no muted type token. Oxlint errors on font size and font color `text-*` classnames under `modules/**`. Media / map / chart / calendar roots use `not-typeset`. Studio page article ([`Article.tsx`](../../apps/studio/components/home/Article.tsx)) wraps TipTap in `typeset typeset-article` for document block flow; `.typeset-article` lives in studio [`globals.css`](../../apps/studio/app/globals.css). Library TextEditorControl does not use it.
