...
const SYSTEM_WORKER_NAME = 'sys_local:production';
...
describe('SelfHostedZerospinApis ordinary production lifecycle', () => {
  it('creates an empty seedless root and activates it before exposing APIs', async () => {
    // 1. The production Worker blocks composed APIs on one complete
    //    self-hosted deploy lifecycle and validates both project-owned keys.
    const selfHostedApis =
      env.SELF_HOSTED_ZEROSPIN_APIS.getByName(SYSTEM_WORKER_NAME);
    const readiness = await selfHostedApis.fetch(
      new Request('https://dev-seeds.test/__zerospin/ready'),
    );
    const readinessBody = await readiness.text();
    expect({ status: readiness.status, body: readinessBody }).toEqual({
      status: 204,
      body: '',
    });
    using secretApis = newSyncRpcSession<ZerospinApis>(RPC_URL);
    const systemApi = secretApis.getSystemApi({
      zerospinSecretKey: 'sk_live_self_hosted_test',
    });
    const secretHello = await systemApi.hello({
      args: [],
      traceContext: null,
    });
    const secretResult = await Effect.runPromise(
      decodeRpc(secretHello.result).pipe(Effect.either),
    );
    expect(Either.isRight(secretResult)).toBe(true);

    // 2. The publishable key is recognized as a production identity but cannot
    //    open the secret-only SystemApi. An unrelated key fails before routing.
    using publishableApis = newSyncRpcSession<ZerospinApis>(RPC_URL);
    const publishableSystemApi = publishableApis.getSystemApi({
      zerospinSecretKey: 'pk_live_self_hosted_test',
    });
    const publishableHello = await publishableSystemApi.hello({
      args: [],
      traceContext: null,
    });
    const publishableResult = await Effect.runPromise(
      decodeRpc(publishableHello.result).pipe(Effect.either),
    );
    expect(Either.isLeft(publishableResult)).toBe(true);
    if (Either.isLeft(publishableResult)) {
      expect(publishableResult.left.code).toBe('publishable-key-not-allowed');
    }

    using invalidApis = newSyncRpcSession<ZerospinApis>(RPC_URL);
    const invalidSystemApi = invalidApis.getSystemApi({
      zerospinSecretKey: 'sk_live_not_this_deployment',
    });
    const invalidHello = await invalidSystemApi.hello({
      args: [],
      traceContext: null,
    });
    const invalidResult = await Effect.runPromise(
      decodeRpc(invalidHello.result).pipe(Effect.either),
    );
    expect(Either.isLeft(invalidResult)).toBe(true);
    if (Either.isLeft(invalidResult)) {
      expect(invalidResult.left.code).toBe('self-hosted-api-key-invalid');
    }

    // 3. First ordinary startup owns a root generation, but it never evaluates
    //    the configured seed Effect and never writes a clean request receipt.
    const stored = await runInDurableObject(selfHostedApis, instance => ({
    ...
    if (
      ...
      throw new Error('Expected the first ordinary production deployment');
    }
  ...
  it('creates a new deploy but reuses the active generation for an identical-model reload', async () => {
    ...
    const selfHostedApis =
      env.SELF_HOSTED_ZEROSPIN_APIS.getByName(SYSTEM_WORKER_NAME);
    const firstReady = await selfHostedApis.fetch(
      new Request('https://dev-seeds.test/__zerospin/ready'),
    );
    expect(firstReady.status).toBe(204);

    // 2. Simulate a new Wrangler code version without changing its SystemSpec.
    const firstDeploy = await runInDurableObject(selfHostedApis, instance => {
    ...
    const secondReady = await env.SELF_HOSTED_ZEROSPIN_APIS.getByName(
      SYSTEM_WORKER_NAME,
    ...
    if (stored.active === undefined || stored.secondDeploy === undefined) {
      throw new Error('Expected the exact-compatible production reload');
    }
  ...
  it('creates and drains into a successor generation for a compatible model-definition change', async () => {
    ...
    if (
      ...
      throw new Error('Expected the compatible migrated production reload');
    }
    ...
    expect(stored.active.activeDeployId).toBe(stored.secondDeploy.id);
    expect(stored.active.transitionSourceDeployId).toBeNull();
    expect(seedTestState.runs).toBe(0);

    const sourceLifecycle = await runInDurableObject(
      env.SYSTEM_REPO.getByName(`sysrepo_${firstDeploy.generationId}`),
      (_instance, state) =>
        state.storage.sql
          .exec<{
            ...
            'SELECT admission, drainFrozenAt, drainedAt FROM generationState WHERE generationId = ?',
            firstDeploy.generationId,
          )
          .one(),
    );
    expect(sourceLifecycle.admission).toBe('drained');
    expect(sourceLifecycle.drainFrozenAt).not.toBeNull();
    expect(sourceLifecycle.drainedAt).not.toBeNull();

    // 5. A crash after source completion but before clearing the dispatch
    //    receipt is recovered by the completed Worker-version mapping. The
    //    already-drained cleanup is retried without allocating another deploy.
    await runInDurableObject(
      env.SELF_HOSTED_ZEROSPIN_APIS.getByName(SYSTEM_WORKER_NAME),
      instance => {
        instance.db
          .update(instance.schema.systemInstance)
          .set({ transitionSourceDeployId: firstDeploy.id })
          .where(
            eq(
              instance.schema.systemInstance.systemWorkerName,
              SYSTEM_WORKER_NAME,
            ),
          )
          .run();
    ...
    await abortAllDurableObjects();
    const cleanupRetry = await env.SELF_HOSTED_ZEROSPIN_APIS.getByName(
      SYSTEM_WORKER_NAME,
    ).fetch(new Request('https://dev-seeds.test/__zerospin/ready'));
    expect(cleanupRetry.status, await cleanupRetry.clone().text()).toBe(204);
    const retriedState = await runInDurableObject(
      env.SELF_HOSTED_ZEROSPIN_APIS.getByName(SYSTEM_WORKER_NAME),
      instance => ({
    ...
    expect(retriedState.instance?.transitionSourceDeployId).toBeNull();
    expect(retriedState.deploys).toHaveLength(2);
  });
  ...
  it('records an incompatible candidate as failed and leaves the prior deploy active', async () => {
    ...
    //    which is a major data-compatibility break in production.
    const firstDeploy = await runInDurableObject(selfHostedApis, instance => {
      const selected = instance.db
        .select()
        .from(instance.schema.deploy)
    ...
    await abortAllDurableObjects();
    const response = await env.SELF_HOSTED_ZEROSPIN_APIS.getByName(
      SYSTEM_WORKER_NAME,
    ).fetch(new Request('https://dev-seeds.test/__zerospin/ready'));
    ...
    expect(message).toContain('before deploying again');
