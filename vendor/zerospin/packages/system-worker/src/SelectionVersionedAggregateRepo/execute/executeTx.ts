...
export const executeTx = makeTx(
  'SelectionVersionedAggregateRepo.executeTx',
  SelectionVersionedAggregateRepoDb,
)(function* (props: {
  inputs: Array<{
    row:
  ...
  const tx = yield* SelectionVersionedAggregateRepoDb.Tx;
  ...
  const state = tx
    ...
    .where(
      eq(selectionVersionedAggregateRepoDbConfig.schema.projectionState.id, 1),
    )
    .get();
  ...
  for (const input of inputs) {
    ...
    if (serviceEntry !== null) {
      const source = tx
        .select()
        .from(selectionVersionedAggregateRepoDbConfig.schema.services)
        .where(
          eq(
            selectionVersionedAggregateRepoDbConfig.schema.services.serviceName,
    ...
    for (const [mutationIndex, mutation] of input.mutations.entries()) {
      if (mutation.operationName === 'replicate') {
        ...
        const source = tx
          ...
          .where(
            eq(
              selectionVersionedAggregateRepoDbConfig.schema.services
                .serviceName,
              serviceName,
            ),
          )
    ...
    if (serviceEntry !== null) {
      tx.update(selectionVersionedAggregateRepoDbConfig.schema.services)
    ...
    tx.insert(selectionVersionedAggregateRepoDbConfig.schema.deltas)
      ...
      .run();
    tx.insert(selectionVersionedAggregateRepoDbConfig.schema.projectionState)
      .values({
        id: 1,
