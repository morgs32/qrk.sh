# Shared selection-versioned aggregate replicas
...
Replace FrontendAggregateRepo and FrontendAggregateChain with SelectionVersionedAggregateRepo (SelectionVAR) and SelectionVersionedAggregateChain (SelectionVAC). Frontends select exact compatible subsets at the capability and connection boundaries. SelectionVAC retains user history indefinitely; SelectionVAR owns current state and its delivery outbox.

## User Stories

1. Frontend authors can change compatible model and contract subsets without publishing a server frontend registry.
2. Frontends sharing a selection path within the same aggregate and version share one replica and history while receiving different subsets.
...
1. Both shared objects use `{ systemId, aggregateId, aggregateName, aggregateVersion, selectionPath }`. Worker configuration supplies systemId; authentication supplies aggregateId and the canonical selectionPath derived from selectionSchema and pattern; admission checks the caller-selected aggregate name and version. Frontend names and locks belong to capabilities and connections.
...
4. SelectionVAR replicates the aggregate's full model set and computes the user-selected graph after aggregate and pinned service replay. Entry and exit from selections produce visibility deltas.
...
8. Snapshot requests supply outstanding command IDs. SelectionVAC resolves only those IDs through the captured userIndex, restricted to the requesting frontend, using an indexed lookup. Return complete outcomes. Remove SelectionVAR's accumulating resolved-command-ID table.
...
The earlier bounded-window proposal and FAC deletion were superseded by retained SelectionVAC history. Snapshot reconciliation therefore reads SelectionVAC, not a second authoritative execution history.

## Implementation Verification

1. Shared SelectionVAR/SelectionVAC identities, Worker exports and bindings, inspection APIs, frontend consumers, and current architecture documentation use the new names and userIndex. Removed aggregate frontend registry access, projection adapters, frontend guard databases, and accumulating resolved-command-ID storage.
