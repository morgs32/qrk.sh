...
/*
 * 1. Prepare every unseen command's aggregate-contract mutations without resource RPCs.
 ...
 */
export const prepareAggregateCommands = Effect.fn(
  'AggregateRepo.prepareAggregateCommands',
)(function* (props: {
  generationId: string;
  ...
  Readonly<{
    preparedCommands: readonly Readonly<{
      ...
      encodedCommand: IEncodedCommand<IAggregateCommand>;
  ...
  Async | CuidFactory
> {
  const { aggregateName, commands, db, generationId } = props;
  ...
  for (const encodedCommand of commands) {
    let command: IAggregateCommand = encodedCommand;
    const preparedMutations = yield* Effect.gen(function* () {
      if (encodedCommand.aggregateName !== aggregateName) {
        return yield* new ZerospinError({
          code: 'system-runtime-aggregate-command-scope-mismatch',
          message: `Aggregate command ${encodedCommand.id} targets ${encodedCommand.aggregateName}, not ${aggregateName}`,
        });
      }
      ...
      if (contract === undefined) {
        return yield* new ZerospinError({
          ...
          message: `Aggregate contract "${encodedCommand.commandName}" was not found`,
        });
      }
      ...
      return { mutations: [...prepared.mutations] };
    }).pipe(Effect.either);
