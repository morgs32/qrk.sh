...
sequenceDiagram
  ...
  participant SelectionVersionedAggregateRepo
  ...
  autonumber 6
  VersionedAggregateChain->>SelectionVersionedAggregateRepo: receiver.receive(...)
  autonumber 7
...
6. VAC fans terminal aggregate entries to SelectionVAR. Initial service copies come from these mutations; SelectionVAR closes a new enrollment's missing retained suffix before committing if its source cursor is already ahead.
   - [`execute.ts`](../../../packages/system-worker/src/SelectionVersionedAggregateRepo/execute/execute.ts) and [`executeTx.ts`](../../../packages/system-worker/src/SelectionVersionedAggregateRepo/execute/executeTx.ts) — Prepares bounded late enrollment outside the transaction and replays successful aggregate mutations without programs or guards.
...
- [`selectionVersionedAggregateRepoFixedDORepoConfig.ts`](../../../packages/system-worker/src/SelectionVersionedAggregateRepo/selectionVersionedAggregateRepoFixedDORepoConfig.ts) — Defines the five-field shared user replica name.

AC retains inputs only. VAR keeps terminal entries pending publication; VAC owns retained execution history after outbox deletion. VAR and SelectionVAR retain one `services.lastIndex` per declared service. Replica-model rows and tombstones establish resource membership and retain their own `serviceIndex`. SelectionVAR also owns the preceding projected graph, both progress indices and pending output. SelectionVAC owns browser output history. There is no downstream frontend materializer or server optimism.
...
- [`selectionVersionedAggregateRepoDbConfig.ts`](../../../packages/system-worker/src/SelectionVersionedAggregateRepo/selectionVersionedAggregateRepoDbConfig.ts) — Defines projection state, source cursors, and the delivery outbox.
...
- [`sourceReplay.node.spec.ts`](../../../packages/system-worker/src/SelectionVersionedAggregateRepo/execute/sourceReplay.node.spec.ts) — Covers late enrollment, duplicate and invalid source delivery, future copies, tombstones, and transaction rollback.
