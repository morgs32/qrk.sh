...
export const prepareAggregateFrontendCommand = Effect.fn(
  'AggregateFrontendRepo.prepareAggregateFrontendCommand',
)(function* (props: {
  db: IDb;
  command:
...
> {
  ...
  if (
    contract === undefined ||
    (contract.version !== props.command.contractVersion &&
      !contract.spec.historicalDefinitions.some(
        definition => definition.version === props.command.contractVersion,
      ))
  ) {
  ...
  for (const guard of guards) {
    ...
    for (const modelName of Object.keys(guard.models)) {
      ...
      if (modelQuery === undefined) {
        return yield* new ZerospinError({
          code: 'aggregate-frontend-guard-model-query-missing',
          message: `AggregateFrontendRepo has no query for guard model "${modelName}"`,
        });
      }
  ...
  const prepared = yield* makeMutations({
    contract,
    models: frontendBinding.controller.models,
  ...
  return {
    encodedPayload: yield* contract.encodePayload({
