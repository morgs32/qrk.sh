...
const selectionVersionedAggregateChainFixedDORepoConfig = makeFixedDORepoConfig(
  {
    abbreviation: systemWorkerAbbreviations.selectionVersionedAggregateChain,
    ...
    managedRuntime,
    dbConfig: Effect.fn('SelectionVersionedAggregateChain.dbConfig')(
      function* ({ key }) {
        const owner =
        ...
        return selectionVersionedAggregateChainDbConfig;
      },
    ),
  },
);

export class SelectionVersionedAggregateChain extends makeFixedDORepo({
  namespaceBinding: 'SELECTION_VERSIONED_AGGREGATE_CHAIN',
  baseClass: Server,
  fixedDORepoConfig: selectionVersionedAggregateChainFixedDORepoConfig,
}) {
