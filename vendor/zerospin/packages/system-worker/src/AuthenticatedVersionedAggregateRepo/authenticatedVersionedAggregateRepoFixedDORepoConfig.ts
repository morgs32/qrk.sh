...
import { authenticatedVersionedAggregateRepoTables } from './authenticatedVersionedAggregateRepoDbConfig.js';

const { system } = config;

/** Name and fixed-schema metadata stay independent of the class and its VAC/AVAC RPC dependencies. */
/*
 * AuthenticatedVersionedAggregateRepo activation resolves its resource schema from the
 ...
 */
export const authenticatedVersionedAggregateRepoFixedDORepoConfig =
  makeFixedDORepoConfig({
    abbreviation: systemWorkerAbbreviations.authenticatedVersionedAggregateRepo,
    ...
    managedRuntime,
    dbConfig: Effect.fn('AuthenticatedVersionedAggregateRepo.dbConfig')(
      function* ({ key }) {
