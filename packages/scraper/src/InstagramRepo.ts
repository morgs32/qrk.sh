    ...
    if (cached !== undefined) {
      if (existingScrape === undefined) {
        const refreshPromise = this.env.BROWSER_HOST.getByName(GLOBAL_BROWSER_HOST_NAME).scrapeInstagram(canonicalUrl).then(async result => {
          ...
          const decoded = await Effect.runPromise(Schema.decodeUnknownEffect(Schema.fromJsonString(InstagramPayloadSchema))(result.right, { onExcessProperty: "preserve" }).pipe(
