...
describe("QRK system", () => {
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const { userV6 } = yield* Effect.promise(() => import("./aggregates/user/UserV6"));
        const authenticate = userV6.authentication.authenticate;
  ...
  it.effect("registers the user aggregate and enforces its authenticated ownership", () =>
    Effect.gen(function* () {
      ...
      expect(system.aggregates.user["6.0.0"].authentication.pattern.source).toBe("/:clerkUserId");
      expect(Object.keys(system.aggregates.user)).toEqual(["6.0.0"]);
      const aggregate = system.aggregates.user["6.0.0"];
  ...
  it.effect("rejects malformed signatures and invalid Clerk tokens", () =>
    Effect.gen(function* () {
      ...
      const authentication = system.aggregates.user["6.0.0"].authentication;
