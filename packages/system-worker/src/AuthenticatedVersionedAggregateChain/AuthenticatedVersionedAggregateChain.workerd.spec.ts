...
import { AuthenticatedVersionedAggregateChain } from './AuthenticatedVersionedAggregateChain.js';
...
it('shares one history across concurrent frontend locks and filters replay and live delivery without cursor gaps', async () => {
  ...
  const repo = await Effect.runPromise(
    AuthenticatedVersionedAggregateChain.getRepo({ key }),
  );
  ...
  for (const connection of connections) {
    ...
    expect(
      JSON.parse(JSON.stringify(paged.commands.map(entry => entry.resolution))),
    ).toEqual(connection.messages.map(entry => entry.resolution));
