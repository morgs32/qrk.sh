...
import {
  ...
  authenticatedVersionedAggregateRepoTables,
...
vi.mock(
  '../../AuthenticatedVersionedAggregateChain/AuthenticatedVersionedAggregateChain.js',
  async importOriginal => {
    const actual =
      await importOriginal<
        typeof import('../../AuthenticatedVersionedAggregateChain/AuthenticatedVersionedAggregateChain.js')
      >();

    Object.assign(actual.AuthenticatedVersionedAggregateChain, {
      getRepo: () =>
        Effect.succeed({
  ...
  const db = await Effect.runPromise(
    makeProvisionedInMemorySqljsDb({
      dbConfig: makeResourceDbConfig({
        otherTables: authenticatedVersionedAggregateRepoTables,
        models: system.aggregates.user['1.0.0']!.models,
      }),
    }).pipe(Effect.provide(AsyncLive)),
  );
  db.insert(authenticatedVersionedAggregateRepoDbConfig.schema.projectionState)
    .values({
      id: 1,
      aggregateIndex: 3,
      userIndex: 10,
    ...
    .run();
  db.insert(authenticatedVersionedAggregateRepoDbConfig.schema.services)
    .values({
      serviceName: 'app',
      lastIndex: 7,
    })
