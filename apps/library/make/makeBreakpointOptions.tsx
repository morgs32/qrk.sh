...
export function makeBreakpointOptions<const SHAPE extends IShape>(props: {
  shape: SHAPE;
  form?: (props: {
  ...
  for (const [name, descriptor] of Object.entries(props.shape)) {
    if (
      ...
      throw new Error(
        `makeBreakpointOptions: ${name} requires a custom form; automatic controls require non-nullable booleans with boolean defaults`,
      );
    }
