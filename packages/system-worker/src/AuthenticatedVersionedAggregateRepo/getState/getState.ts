 ...
 */
export const getState = Effect.fn(
  'AuthenticatedVersionedAggregateRepo.getState',
)(function* (
  props: Parameters<typeof catchup>[0] & {
    ...
    serviceSubscriber: (sourceKey: {
      ...
      ReturnType<
        AuthenticatedVersionedAggregateRepo['aggregateReplicaFanoutQueueSubscriber']
      >,
      'catchup'
    >;
    requested: {
    ...
    deltas: Pick<AuthenticatedVersionedAggregateRepo['deltas'], 'drain'>;
  },
) {
  const { key, requested } = props;

  // 1 — compare aggregateId, aggregateName, and selectionPath with the bound key
  if (
  ...
  const chain = yield* AuthenticatedVersionedAggregateChain.getRepo({ key });
  const published = yield* makeAsync<
    Awaited<ReturnType<AuthenticatedVersionedAggregateChain['getCommands']>>
  >(() =>
    chain.getCommands({
      afterUserIndex: 0,
      reconcile: {
  ...
  // 6 — reject a AVAC tip behind the captured snapshot index
  ...
  return yield* Schema.decodeUnknownEffect(
    Schema.toType(
      AggregateFrontendSyncStateSchema.mapFields(
