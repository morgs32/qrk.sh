 ...
 */
export const getServiceAdmittedChains = Effect.fn(
  'SystemApi.getServiceAdmittedChains',
)(function* (props: {
  ...
  return yield* makeApiHandler({
    name: 'SystemApi.getServiceAdmittedChains',
    ...
    handler: () =>
      Effect.gen(function* () {
        ...
        // 5 — decode SystemRepo.getRepoRegistrations for ServiceAdmittedChain
        return yield* makeAsync(() =>
          systemRepo.getRepoRegistrations({ repoType: 'ServiceAdmittedChain' }),
        ).pipe(Effect.flatMap(decodeRpc));
      }).pipe(
        Effect.withSpan('SystemApi.getServiceAdmittedChains', { root: true }),
      ),
  })(request).pipe(
