...
export function makeTableProvisioningStatements(
  ...
  const columnLines = tableConfig.columns.map(column => {
    ...
    if (typeof column.default === 'number') {
      ...
      constraints.push(`DEFAULT ${column.default.getTime()}`);
...
export function makeTableProvisioningSQL(table: SQLiteTable): string {
  return makeTableProvisioningStatements(table).join('\n');
}
