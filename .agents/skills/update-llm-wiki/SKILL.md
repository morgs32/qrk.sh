...
description: >-
  ...
  $patterns or Zerospin's first-party pattern profile with @bad
  JSDoc tags.
---
...
Invoke `$patterns` for shared guidance. Its
...
3. **Check existing patterns** by invoking `$patterns` and searching `references/patterns/index.md`, then searching the pinned [`vendor/zerospin/llm-wiki/patterns/index.md`](../../../vendor/zerospin/llm-wiki/patterns/index.md). Skip anything already stated clearly.
...
3. **Pick the right source**
   - Repo-agnostic conventions belong in the source `morgs32/llm-wiki` repository under `skills/patterns/references/patterns/{functions,naming,effect,rpc,runtime,tooling,...}`. Invoke `$patterns` and search its logical `references/patterns/index.md` first.
...
4. **Keep indexes in sync**
   ...
   Update the matching source index: `skills/patterns/references/patterns/index.md` in `morgs32/llm-wiki`, or `llm-wiki/patterns/index.md` in `morgs32/zerospin`. Update the source repository's `AGENTS.md` routing when the new topic needs an explicit route.

5. **Write mock TypeScript patterns from the example**

   Shared pattern format is documented at the logical `$patterns` path `references/patterns/README.md`.
...
7. **Publish the source, then refresh the consumer**
   - Shared guidance: in `morgs32/llm-wiki`, use its repo-local `$update-morgs32-llm-wiki` workflow to publish and merge the source pull request. Verify the commit on remote `main` before using `skills/patterns/scripts/configure.mjs` to refresh `$patterns`; never install or refresh from an unmerged branch.
   - Zerospin guidance: land the source change in `morgs32/zerospin`, then use [`update-vendor`](../update-vendor/SKILL.md) to update the whole `vendor/zerospin` subtree; do not update its nested `llm-wiki` independently.
