...
describe('DevZerospinApis explicit-clean lifecycle', () => {
  it('keeps one bound capability usable across later calls on the persistent WebSocket transport', async () => {
    ...
    using apis = newWebSocketRpcSession<ZerospinApis>(response.webSocket!);
    using systemApi = await apis.getSystemApi({
      zerospinSecretKey: 'zerospin-dev-test',
    });

    const first = await systemApi.hello({
      args: [],
      traceContext: null,
    });
    expect(first.result._tag).toBe('Right');

    const second = await systemApi.hello({
      args: [],
      traceContext: null,
    });
  ...
  it('retains an older clean generation when a later clean request creates another detached root', async () => {
    ...
    // 3. The second clean skips compatibility and lineage replay, but freezes
    //    the active source before seeding a detached root generation.
    await abortAllDurableObjects();
    const secondReady = await env.DEV_ZEROSPIN_APIS.getByName(
      SYSTEM_WORKER_NAME,
    ).fetch(new Request('https://dev-seeds.test/__zerospin/ready'));
    ...
    const firstGenerationLifecycle = await runInDurableObject(
    ...
    expect(firstGenerationLifecycle.admission).toBe('drained');
    expect(firstGenerationLifecycle.drainFrozenAt).not.toBeNull();
    expect(firstGenerationLifecycle.drainedAt).not.toBeNull();
  });
