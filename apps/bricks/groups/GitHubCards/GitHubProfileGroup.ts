...
export const githubGroup = makeGroup({
  groupName: "github",
  groupLabel: "GitHub",
  groupDescription: "Profile and repository cards from GitHub.",
  catalogs: {
    profile: makeCatalog({
      catalog: "profile",
      catalogName: "Profile",
      catalogDescription: "A GitHub profile card.",
      configuration: makeFetcherConfiguration({
        catalogOptionsShape: {
          url: primitives.text({ defaultValue: "https://github.com/morgs32" }),
        },
        fetcher: async ({ api, catalogOptions, setData }) => {
          const result = await api.githubRepo().getProfile(catalogOptions.url);
    ...
    repo: makeCatalog({
      ...
      catalog: "repo",
      catalogName: "Repo",
      catalogDescription: "A GitHub repository card.",
      order: 1,
