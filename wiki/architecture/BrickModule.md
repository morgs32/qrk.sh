...
sources:
  ...
  - path: apps/library/app/routes.ts
    ...
    lines: 27-56
  - path: apps/library/app/routes/ModulePage.tsx
    sha: 49a411f8d6f581614b646d77dafd3f2fcfa2f157
    lines: 10-13
  - path: apps/library/app/routes/ModuleConfiguration.tsx
    sha: 29312bdefd07b571d590de20fdfa941325e4d742
    lines: 32-43
  - path: apps/library/app/routes/BrickPage.tsx
...
3. Workbench navigation hits [`routes.ts`](../../apps/library/app/routes.ts) `modules/:moduleId` or `bricks/:moduleId`.
...
sequenceDiagram
  ...
  alt missing id or hash miss
    ...
    autonumber 8
    ModuleConfiguration->>modulesHash: modulesHash[moduleId]
    autonumber 9
    modulesHash-->>ModuleConfiguration: brickModule
    autonumber 10
    ModuleConfiguration->>Brick: brickModule.component(...)
    autonumber 11
...
4. The configuration parent loader admits only registered `params.moduleId`.
   - [`routes.ts:27-41`](../../apps/library/app/routes.ts#L27-L41) — `path: "modules/:moduleId"` lazy `ModulePage` plus nested `ModuleConfiguration`. (`apps/library/app/routes.ts:27-41`)
   - [`ModulePage.tsx:10-13`](../../apps/library/app/routes/ModulePage.tsx#L10-L13) — 404 when `params.moduleId` is missing or not in `modulesHash`. (`apps/library/app/routes/ModulePage.tsx:10-13`)
5. A miss on that lookup is `undefined`.
   - [`ModulePage.tsx:12-12`](../../apps/library/app/routes/ModulePage.tsx#L12) — `if (!modulesHash[params.moduleId])`. (`apps/library/app/routes/ModulePage.tsx:12-12`)
6. The loader throws a 404 `Response`.
   - [`ModulePage.tsx:11-12`](../../apps/library/app/routes/ModulePage.tsx#L11-L12) — `throw new Response("Not found", { status: 404 })`. (`apps/library/app/routes/ModulePage.tsx:11-12`)
   - [`BrickPage.tsx:14-17`](../../apps/library/app/routes/BrickPage.tsx#L14-L17) — the same hash check on `bricks/:moduleId`. (`apps/library/app/routes/BrickPage.tsx:14-17`)
7. A hit means the loader returns `null` and the child route renders.
   - [`ModulePage.tsx:13-13`](../../apps/library/app/routes/ModulePage.tsx#L13) — `return null` after the hash hit. (`apps/library/app/routes/ModulePage.tsx:13-13`)
8. The configuration pane looks up the same key as `brickModule`.
   - [`ModuleConfiguration.tsx:32-35`](../../apps/library/app/routes/ModuleConfiguration.tsx#L32-L35) — 404 without `params.moduleId`, then `brickModule = modulesHash[moduleId]`. (`apps/library/app/routes/ModuleConfiguration.tsx:32-35`)
   - [`ModuleConfiguration.tsx:37-39`](../../apps/library/app/routes/ModuleConfiguration.tsx#L37-L39) — pane 404 when the hash miss still happens after the loader. (`apps/library/app/routes/ModuleConfiguration.tsx:37-39`)
   - [`BrickPage.tsx:20-24`](../../apps/library/app/routes/BrickPage.tsx#L20-L24) — standalone preview binds `brickModule = modulesHash[params.moduleId]`. (`apps/library/app/routes/BrickPage.tsx:20-24`)
...
9. The hash returns that `IModule`.
   - [`ModuleConfiguration.tsx:35`](../../apps/library/app/routes/ModuleConfiguration.tsx#L35) — `const brickModule = modulesHash[moduleId]`. (`apps/library/app/routes/ModuleConfiguration.tsx:35`)
10. Render uses `brickModule.component` as `Brick`.
    - [`ModuleConfiguration.tsx:42-43`](../../apps/library/app/routes/ModuleConfiguration.tsx#L42-L43) — `const brick = brickModule` then `BrickComponent = brick.component`. (`apps/library/app/routes/ModuleConfiguration.tsx:42-43`)
