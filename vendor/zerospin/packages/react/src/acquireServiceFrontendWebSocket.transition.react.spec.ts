...
const createServiceFrontendWebSocketTicketMock = vi.hoisted(() => vi.fn());
const fetchServiceFrontendStateMock = vi.hoisted(() => vi.fn());

vi.mock('@zerospin/frontend/createServiceFrontendWebSocketTicket', () => ({
  createServiceFrontendWebSocketTicket:
    createServiceFrontendWebSocketTicketMock,
}));
...
const frontend = makeServiceFrontendController({
  systemName: 'direct-service-transition-system',
  serviceName: 'catalog',
  actorName: 'viewer',
...
beforeEach(() => {
  vi.useFakeTimers();
  createServiceFrontendWebSocketTicketMock.mockReset();
...
describe('acquireServiceFrontendWebSocket direct lineage transitions', () => {
  it('repairs from bound full state after a client-detected message failure before reconnecting', async () => {
    ...
    createServiceFrontendWebSocketTicketMock.mockReturnValue(
      Effect.succeed({
        ticket: 'repair-service-ticket',
        ...sourceIdentity,
      }),
    );
    fetchServiceFrontendStateMock.mockReturnValue(
    ...
    const releasePromise = Effect.runPromise(
      acquireServiceFrontendWebSocket({
        ...
        handleServiceFrontendLineageBlock: () => Effect.void,
    ...
    sockets[0]?.emit(
      ...
      new MessageEvent('message', {
        data: JSON.stringify({
          type: 'not-a-valid-service-frontend-message',
        }),
      }),
    );

    await vi.waitFor(() => expect(replaceFrontendState).toHaveBeenCalledOnce());
    expect(fetchServiceFrontendStateMock).toHaveBeenCalledOnce();
  ...
  it('rejects an ordinary service block from another generation before application', async () => {
    const frontendIndex = 4;
    const statuses: string[] = [];
    const releaseSourceFrontendApi = vi.fn();
    const handleServiceFrontendLineageBlock = vi.fn(() => Effect.void);
    createServiceFrontendWebSocketTicketMock.mockReturnValue(
      Effect.succeed({
        ticket: 'source-service-ticket',
        ...sourceIdentity,
      }),
    );
      ...
      new MessageEvent('message', {
        data: JSON.stringify({
          ...
          sync: {
            ...
            serviceName: sourceIdentity.serviceName,
    ...
    await vi.waitFor(() => expect(statuses.at(-1)).toBe('repairing'));
    expect(handleServiceFrontendLineageBlock).not.toHaveBeenCalled();
    expect(frontendIndex).toBe(4);
  ...
  it('rejects a service generation boundary with the wrong predecessor before application', async () => {
    const frontendIndex = 4;
    const statuses: string[] = [];
    const releaseSourceFrontendApi = vi.fn();
    const handleServiceFrontendLineageBlock = vi.fn(() => Effect.void);
  ...
  it('consumes the source boundary before switching service authority and reconnecting at the target watermark', async () => {
    let frontendIndex = 4;
    const order: string[] = [];
    const statuses: string[] = [];
    const releaseSourceFrontendApi = vi.fn();
    ...
    createServiceFrontendWebSocketTicketMock
      .mockReturnValueOnce(
        Effect.succeed({
          ticket: 'successor-service-ticket',
          ...targetIdentity,
        }),
      )
      .mockReturnValueOnce(
        Effect.succeed({
          ticket: 'target-service-ticket',
          ...targetIdentity,
        }),
      );
    ...
    const releasePromise = Effect.runPromise(
      acquireServiceFrontendWebSocket({
        ...
        getFrontendIndex: () => frontendIndex,
        replaceFrontendState: () => Effect.void,
        handleServiceFrontendLineageBlock: lineageBlock =>
          Effect.sync(() => {
            order.push('boundary');
            frontendIndex =
    ...
    sockets[0]?.emit(
      ...
      new MessageEvent('message', {
        data: JSON.stringify({
          ...
          generationId: targetIdentity.generationId,
          serviceName: targetIdentity.serviceName,
  ...
  it('retains source service authority and suppresses reconnect when target code does not match', async () => {
    let frontendIndex = 4;
    const statuses: string[] = [];
    const releaseSourceFrontendApi = vi.fn();
    createServiceFrontendWebSocketTicketMock.mockReturnValue(
  ...
  it('keeps the read-only source stream current while a same-generation ticket requires newer code', async () => {
    let frontendIndex = 4;
    const statuses: string[] = [];
    const releaseFrontendApi = vi.fn();
    const transitionToTarget = vi.fn(() => Effect.succeed(null));
        ...
        data: JSON.stringify({
          ...
          sync: {
            ...
            frontendBlock: {
              ...
              frontendName: sourceIdentity.frontendName,
              frontendIndex: 5,
              lastServiceCursor: 'svcur_version_5',
              delta: { inserted: [], updated: [], deleted: [] },
            },
          },
        }),
    ...
    expect(fetchServiceFrontendStateMock).not.toHaveBeenCalled();
    await Effect.runPromise(release);
    expect(releaseFrontendApi).toHaveBeenCalledOnce();
  });

  it('replaces a dead service capability without applying replacement state', async () => {
    const statuses: string[] = [];
    const releaseSourceFrontendApi = vi.fn();
      ...
      .mockReturnValueOnce(
        Effect.fail(
          new ZerospinError({
            ...
            message: 'The original service capability transport is closed',
          }),
        ),
      )
      .mockReturnValueOnce(
        Effect.succeed({
          ticket: 'regained-service-ticket',
          ...sourceIdentity,
        }),
      );
    ...
    await vi.waitFor(() => expect(sockets).toHaveLength(1));
    expect(
      createServiceFrontendWebSocketTicketMock.mock.calls[0]?.[0].frontendApi,
    ).toBe(mockFrontendApi);
    expect(
      createServiceFrontendWebSocketTicketMock.mock.calls[1]?.[0].frontendApi,
  ...
  it('preserves the readable service database and stops at update-required when reauthentication finds only a newer version', async () => {
    const statuses: string[] = [];
    const releaseSourceFrontendApi = vi.fn();
    const replaceFrontendState = vi.fn(() => Effect.void);
    ...
    expect(releaseSourceFrontendApi).not.toHaveBeenCalled();
    await vi.advanceTimersByTimeAsync(30_000);
    expect(createServiceFrontendWebSocketTicketMock).toHaveBeenCalledOnce();

    await Effect.runPromise(release);
    expect(releaseSourceFrontendApi).toHaveBeenCalledOnce();
  });

  it('fails the direct service session closed when transport regain rejects its local signature', async () => {
    const statuses: string[] = [];
    const releaseSourceFrontendApi = vi.fn();
  ...
  it('retries service transport and operational reauthentication failures until a fresh capability succeeds', async () => {
    const releaseSourceFrontendApi = vi.fn();
    const releaseRegainedFrontendApi = vi.fn();
    const handleAuthorityFailure = vi.fn(() => Effect.void);
    const regainedFrontendApi = { ...mockFrontendApi };
    const transportFailure = new ZerospinError({
      code: 'service-frontend-admission-transport-failed',
      message: 'Service authority remains unreachable',
    });
    const operationalFailure = new ZerospinError({
      code: 'service-frontend-state-required',
      message: 'Service authority requested an ordinary state repair',
    });
    ...
    createServiceFrontendWebSocketTicketMock
      ...
      .mockReturnValueOnce(
        Effect.succeed({
          ticket: 'eventually-regained-service-ticket',
          ...sourceIdentity,
        }),
      );
