...
export const Route = createFileRoute("/bricks/$collectionName/$brickName")({
  loader: ({ params }) => {
    ...
    const brick = Object.values(collection.bricks).find(
      (candidate) => candidate.def.name === params.brickName,
    );
...
function BrickPage() {
  ...
  const brick = findCollectionBrick(brickDef);
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
        ...
        <aside className="rounded-xl border border-zinc-300 bg-white p-5">
          ...
          <dl className="grid grid-cols-[7rem_1fr] gap-x-3 gap-y-2 text-sm">
            ...
            <dt className="text-zinc-500">Brick</dt>
            <dd className="m-0 font-mono">{brick.def.name}</dd>
...
function BrickNotFound() {
  return (
    <main className="mx-auto max-w-3xl p-6" data-testid="brick-not-found">
      <h1>Brick not found</h1>
      <p>The requested collection and brick name pair is not registered in the catalog.</p>
