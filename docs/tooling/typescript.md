...
```sh
pnpm --filter @qrk.sh/studio run tsc
```
...
- **Good**: colocate in the route’s `page.tsx` (for example parallel routes under `apps/studio/app/[username]/site/[siteId]/page/[pageId]/@leftDrawer/…`):
...
- **Good**: put `satisfies Schema.Schema<…>` on the **`Schema.Struct`** that describes the **parsed object** (see `BrickDragDefFromJsonStringSchema` in [`apps/studio/components/home/useBrickDrawerStore.ts`](../../apps/studio/components/home/useBrickDrawerStore.ts) and the `parseJson` subsection below).
...
See [`apps/studio/components/home/useBrickDrawerStore.ts`](../../apps/studio/components/home/useBrickDrawerStore.ts) (`parseBrickDefFromDataTransfer`).
