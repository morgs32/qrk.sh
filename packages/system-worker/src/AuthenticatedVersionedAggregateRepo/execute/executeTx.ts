...
import {
  ...
  authenticatedVersionedAggregateRepoDbConfig,
} from '../authenticatedVersionedAggregateRepoDbConfig.js';
...
export const executeTx = makeTx(
  'AuthenticatedVersionedAggregateRepo.executeTx',
  AuthenticatedVersionedAggregateRepoDb,
)(function* (props: {
  inputs: Array<{
    row:
  ...
  const matched = yield* Effect.try({
    try: () =>
      createMatcher(aggregate.authentication.pattern).match(
  ...
  const selectedClaims = yield* Schema.decodeUnknownEffect(
    ...
    mapParseError({
      code: 'selection-path-invalid',
      prefix: 'Invalid selection fields',
    }),
  );
  ...
  if (
    matched === null ||
    createHref(aggregate.authentication.pattern, matched.params) !==
      key.selectionPath
  ) {
  ...
  const tx = yield* AuthenticatedVersionedAggregateRepoDb.Tx;
  // 2 — decode the stored graph and restore its aggregate cursor
  const state = tx
    .select()
    .from(authenticatedVersionedAggregateRepoDbConfig.schema.projectionState)
    .where(
      eq(
        authenticatedVersionedAggregateRepoDbConfig.schema.projectionState.id,
        1,
      ),
    )
    .get();
  ...
  for (const input of inputs) {
    ...
    if (serviceEntry !== null) {
      const source = tx
        .select()
        .from(authenticatedVersionedAggregateRepoDbConfig.schema.services)
        .where(
          eq(
            authenticatedVersionedAggregateRepoDbConfig.schema.services
              .serviceName,
    ...
    if (serviceEntry !== null) {
      tx.update(authenticatedVersionedAggregateRepoDbConfig.schema.services)
    ...
    tx.insert(authenticatedVersionedAggregateRepoDbConfig.schema.deltas)
      .values({
      ...
      .run();
    tx.insert(
      authenticatedVersionedAggregateRepoDbConfig.schema.projectionState,
    )
      .values({
        id: 1,
