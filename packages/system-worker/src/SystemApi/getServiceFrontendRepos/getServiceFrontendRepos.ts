    ...
    handler: () =>
      Effect.gen(function* () {
        ...
        const encoded = yield* makeAsync(() =>
          systemWorker.getServiceFrontendRepos({
            generationId: authResults.generationId,
          }),
        );
        return yield* decodeRpc(encoded);
      }).pipe(
        Effect.withSpan('SystemApi.getServiceFrontendRepos', { root: true }),
      ),
  })(props.request).pipe(
