...
export const getServiceFrontendBlockRepos = Effect.fn(
  'SystemApi.getServiceFrontendBlockRepos',
)(function* (props: {
  ...
  return yield* makeApiHandler({
    name: 'SystemApi.getServiceFrontendBlockRepos',
    ...
    handler: () =>
      Effect.gen(function* () {
        ...
        const encoded = yield* makeAsync(() =>
          systemWorker.getServiceFrontendBlockRepos({
            generationId: authResults.generationId,
          }),
        );
        return yield* decodeRpc(encoded);
      }).pipe(
        Effect.withSpan('SystemApi.getServiceFrontendBlockRepos', {
          root: true,
        }),
      ),
  })(props.request).pipe(
