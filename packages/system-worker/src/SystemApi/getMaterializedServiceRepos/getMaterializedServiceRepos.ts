...
export const getMaterializedServiceRepos = Effect.fn('SystemApi.getMaterializedServiceRepos')(
  function* (props: {
    ...
    return yield* makeApiHandler({
      name: 'SystemApi.getMaterializedServiceRepos',
      ...
      handler: () =>
        Effect.gen(function* () {
          ...
          return yield* makeAsync(() =>
            systemRepo.getRepoRegistrations({ repoType: 'MaterializedServiceRepo' }),
          ).pipe(Effect.flatMap(decodeRpc));
        }).pipe(Effect.withSpan('SystemApi.getMaterializedServiceRepos', { root: true })),
    })(request).pipe(
