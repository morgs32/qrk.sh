...
export const getReadiness = Effect.fn('SystemRepo.getReadiness')(
  function* (props: {
    db: IDb;
    ...
    if (
      ...
      const productionDeployId = productionDeploy.id;
      yield* makeAsync(() => props.scheduleActivation(productionDeployId)).pipe(
        Effect.either,
      );
    ...
    if (
      ...
      active.workerVersionId === props.executingWorkerVersionId
    ) {
      return;
    }
    return yield* new ZerospinError({
      ...
      message:
        'The executing Worker bundle has not completed System activation',
      status: 503,
      extra: {
        activeDeployId: active?.id ?? null,
        activeWorkerVersionId: active?.workerVersionId ?? null,
