 ...
 * Upserts the post-publish AccountRepo account block row, encoding JSON columns
...
export const upsertAccountBlock = Effect.fn('AccountRepo.upsertAccountBlock')(
  function* (props: {
    accountBlock: IAccountBlockOutboxRecord;
    db: IDb;
  }): Effect.fn.Return<void, IAnyError> {
    const { accountBlock, db } = props;
    ...
    db.insert(accountRepoDrizzleSchemas.accountBlockOutbox)
      .values({
